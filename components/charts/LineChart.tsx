...
export default function LineChart({ stream, data, compact = false }: LineChartProps) {
  const { getESP32 } = useConfigStore()
  
  const chartData = useMemo(() => {
    // Prendre les 50 derniers points pour la performance (moins en mode compact)
    const recentData = data.slice(compact ? -20 : -50)
